...
if x==7:
    ...
    print('anything could be the case')
...
Lets unpack the `for` loop syntax. First, we declare a for loop by using the `for` keyword. Next, we specify the variable name that we want to use to keep track of the step or current iteration we are on when our for loop runs. In our example we used the variable name of `x`. Proceeding our iteration variable we declare the `in` keyword followed by the name of the list or range we want to loop through.  
...
# create a variable to add things onto
total = 0
...
for player in sports_stats:
    ...
    # Calculate goal percentage (num_goals/num_shots)
    goal_pct = player['goals'] / player['shots']
    
    # Print the player name if goal_pct > 0.5
    if goal_pct > 0.5:
        print(player['name'])
```
...
```
      ...
      File "/var/folders/49/k90p8vsd6tz1975g9n2s_tkw0000gq/T/ipykernel_37404/912176141.py", line 6
        else:
        ^
...
```
    ...
    I know what should happen if x is not larger than 5
...
```
    ...
    It doesn't do much except printing
    But whenever you want to call these print statements together you can just call this function
...
```
    ...
    You've entered 3.4 as the input
    3.4 km is 3400.0 meters
...
```
    ...
    You've entered a as the input
    a km is  meters
...
```
    ...
    Person A's BMI is greater than, or equal to, 20
...
def my_func(mylist):
    ...
    if type(mylist)==str:
        print('input argument must be of type list')
...
```python
def distance_converter(input_distance: int, input_units='km'):
    '''
    Takes a numeric input (miles or km distance) and converts distance to the other unit
    
    Parameters:
    -----------
    input_distance: numeric, the distance to be converted
    input_units: string, the units of the input distance (either 'km' or 'miles')
    
    Returns:
    --------
    output_distance: numeric, the converted distance
    
    Examples:
    ---------
    >>> distance_converter(1)
    0.625
    >>> distance_converter(10, 'miles')
    160
    '''
    
    # Assert that input units is either 'km' or 'miles'
    assert input_units=='km' or input_units=='miles', 'input_units must be either "km" or "miles"'
    # Assert that input distance is numeric
    assert type(input_distance)==int or type(input_distance)==float, 'input_distance must be numeric'
    
    if input_units == 'km':
        output_distance = input_distance / 1.6   # we are converting km to miles
    else:
        output_distance = input_distance * 1.6   # we are converting miles to km
...
```python
help(distance_converter)
```

    Help on function distance_converter in module __main__:
    
    distance_converter(input_distance: (<class 'int'>, <class 'float'>), input_units='km')
        Takes a numeric input (miles or km distance) and converts distance to the other unit
...
```
    ...
    /var/folders/49/k90p8vsd6tz1975g9n2s_tkw0000gq/T/ipykernel_37404/3750458404.py in <module>
    ----> 1 distance_converter('abc')
    

    /var/folders/49/k90p8vsd6tz1975g9n2s_tkw0000gq/T/ipykernel_37404/4255418250.py in distance_converter(input_distance, input_units)
         23     assert input_units=='km' or input_units=='miles', 'input_units must be either "km" or "miles"'
         24     # Assert that input distance is numeric
    ---> 25     assert type(input_distance)==int or type(input_distance)==float, 'input_distance must be numeric'
         26 
         27     if input_units == 'km':


    AssertionError: input_distance must be numeric
...
```python
word_list = ['apple','banana','cherry','onion']
```


```python
def vowel_finder(input_list):
    ...
    for word in input_list:
        ...
        # check if it begins with a vowel, append to output list if so
